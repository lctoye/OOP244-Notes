/* BTP200.141 Assignment 2, 3, 4 Test Main
...
*  Version 1.03 - inconsistencies in error messages fixed - Mar 11 2014 CS
*
...
int EANTests(const Prefix& list, int& testNo, int* passed, int* tested) {
    ...
    if(!strcmp(strb, "9789070002043")) {
        testFailed("toStr didn\'t return a string different from 9789070002043",
         ++testNo, &ok, tested);
    } else
...
int OrderTests(const Prefix& list, int& testNo, int* passed, int* tested) {
    ...
    if(!(order.getEAN() == ean) || order.outstanding() != 0) {
        testFailed("Order(EAN(9789070002043, list)), outstanding()",
         ++testNo, &ok, tested);
    } else
        testPassed("Order(EAN(9789070002043, list)), outstanding()", ++testNo, passed,
         tested);

    order.add(5);
    if(!(order.getEAN() == ean) || order.outstanding() != 0) {
        testFailed("Order(EAN(9789070002043, list)), add(5), outstanding()",
         ++testNo, &ok, tested);
    } else
        testPassed("Order(EAN(9789070002043, list)), add(5), outstanding()", ++testNo, passed,
         tested);
...
int SpecialOrderTests(const Prefix& list, int& testNo, int* passed, int* tested) {
    ...
    //
    if(order0.has(EAN()) == 0) {
        testFailed("SpecialOrder(), has(EAN()) did not return true",
         ++testNo, &ok, tested);
    } else
        testPassed("SpecialOrder(), has(EAN()) returned true", ++testNo,
         passed, tested);
    ...
    //
    if(order1.has(EAN("9789070002046", list)) == 0 || order1.outstanding() != 0) {
        testFailed("has(EAN(\"9789070002046\", )), outstanding()",
         ++testNo, &ok, tested);
    } else
        testPassed("has(\"9789070002046\"), outstanding()", ++testNo, passed,
         tested);

    if(order2.has(EAN("9789070002046", list)) == 0 || order2.outstanding() != 5) {
        testFailed("has(EAN(\"9789070002046\", )), augment(5), outstanding()",
         ++testNo, &ok, tested);
    } else
        testPassed("has(\"9789070002046\"), augment(5), outstanding()", ++testNo, passed,
         tested);
    ...
    */
    ...
    //
    if(order5.has(EAN("9789070002043", list)) == 0 || order5.outstanding() != 0) {
        testFailed("has(EAN(\"9789070002043\", )), outstanding()",
         ++testNo, &ok, tested);
    } else
        testPassed("has(\"9789070002043\"), outstanding()", ++testNo, passed,
         tested);

    if(order6.has(EAN("9789070002043", list)) == 0 || order6.outstanding() != 0) {
        testFailed("has(EAN(\"9789070002043\", )), augment(5), outstanding()",
         ++testNo, &ok, tested);
    } else
        testPassed("has(\"9789070002043\"), augment(5), outstanding()", ++testNo, passed,
         tested);
    ...
    if(!order6.has(EAN())) {
        ...
        testPassed("Order(), has(EAN()) returned true", ++testNo, passed,
         tested);

    // assignment operator
    //
    order0 = order1;
    if(!(order0.has(EAN("9789070002046", list)) && order0.outstanding() == 0)) {
        testFailed("A.O. - has(\"9789070002046\"), outstanding()", ++testNo, &ok,
         tested);
    } else
        testPassed("A.O. - has(\"9789070002046\")), outstanding()", ++testNo,
         passed, tested);
    ...
    if(!(s.has(EAN("9789070002046", list)) && s.outstanding() == 1)) {
        testFailed("C.C. - has(\"9789070002046\"), outstanding()", ++testNo, &ok,
         tested);
    } else
        testPassed("C.C. - has(\"9789070002046\")), outstanding()", ++testNo,
         passed, tested);

    // constructor - safe state
    //
    SpecialOrder c(EAN("9789070002043", list), "constructor");
    if(!(c.has(EAN()) && c.outstanding() == 0)) {
        testFailed("Ctor - has(\"9789070002043\"), outstanding()", ++testNo, &ok,
         tested);
    } else
        testPassed("Ctor - has(\"9789070002043\")), outstanding()", ++testNo,
         passed, tested);
